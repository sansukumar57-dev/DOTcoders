...
export const useKolamURLParams = (): KolamURLParams => {
	...
	return {
		size: Math.max(3, Math.min(15, parseInt(getParam('size', '7')))),
		duration: Math.max(1000, Math.min(30000, parseInt(getParam('duration', '10000')))),
		background: getParam('background', '#fef3c7'),
		brush: getParam('brush', '#92400e'),
		initialAutoAnimate: getParam('initial-auto-animate', 'false') === 'true'
	};
};
